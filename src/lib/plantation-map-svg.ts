...
function buildSvg(blocks: FieldRow[], style: SvgStyle, opts: BuildOpts = {}): PlantationSvg | null {
  ...
  const svgBlocks: SvgBlock[] = blocks.map((b) => {
    ...
    // block prints ALL FOUR facts on every sheet style — non-negotiable, and
    // highlight sheets are no different (nobody knows what a plan printout
    // ends up used for): whatever can't fit in its block goes to a
    // leader-line callout, same as everywhere else.
