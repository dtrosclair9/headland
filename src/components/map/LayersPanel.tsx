...
export default function LayersPanel({
  ...
  const matched = useMemo(
    () => (active ? fields.filter((f) => fieldMatchesFilter(f, filter)) : fields),
    ...
    <div className="flex-1 overflow-y-auto">
      ...
      <div className={`px-4 py-3 border-b border-gray-100 ${active ? 'bg-accent/10' : 'bg-gray-50'}`}>
        <div className="flex items-baseline justify-between gap-2">
          <div>
            <p className="text-xs uppercase tracking-wider text-gray-500 font-semibold">
              {active ? 'Matching selection' : 'All blocks'}
            </p>
        ...
        {active && matched.length > 0 && (
          <a
            href={`/blocks/print?ids=${matched.map((f) => f.id).join(',')}${isSpray ? '&style=spray' : ''}`}
            target="_blank"
            rel="noreferrer"
          ...
          >
            Print these {matched.length} block{matched.length === 1 ? '' : 's'} →
          </a>
        )}
        {!active && (
