...
// ── GLOBAL grid fit: one rigid transform (translate+rotate) for the WHOLE
// plantation, so the tessellation moves as a unit — fixes the common offset,
// cannot open gaps between neighbours. Sample points from every block edge.
const allPts = []
let gcx = 0, gcy = 0, gn = 0
for (const block of blocks) for (const ring of block.rings) {
  for (let k = 0; k < ring.length - 1; k++) {
    const ax = px(ring[k][0]), ay = py(ring[k][1])
    const bx = px(ring[k+1][0]), by = py(ring[k+1][1])
    gcx += ax; gcy += ay; gn++
    const len = Math.hypot(bx-ax, by-ay), steps = Math.max(1, Math.round(len/6))
    for (let sp = 0; sp < steps; sp++) { const t = sp/steps; allPts.push([ax+(bx-ax)*t, ay+(by-ay)*t]) }
  }
}
gcx /= gn; gcy /= gn
const gcost = (dx, dy, deg) => {
  const r=(deg*Math.PI)/180, c=Math.cos(r), s=Math.sin(r); let sum=0
  for (const [x,y] of allPts) { const rx=gcx+(x-gcx)*c-(y-gcy)*s+dx, ry=gcy+(x-gcx)*s+(y-gcy)*c+dy; sum+=sampleD(rx,ry) }
  return sum/allPts.length
}
const gbase = gcost(0,0,0)
let gbest = { dx:0, dy:0, deg:0, c:gbase }
for (let dx=-40; dx<=40; dx+=4) for (let dy=-40; dy<=40; dy+=4) for (let deg=-3; deg<=3; deg+=1) {
  const c=gcost(dx,dy,deg); if (c<gbest.c) gbest={dx,dy,deg,c}
}
const g0=gbest
for (let dx=g0.dx-3; dx<=g0.dx+3; dx++) for (let dy=g0.dy-3; dy<=g0.dy+3; dy++) for (let deg=g0.deg-1; deg<=g0.deg+1; deg+=0.25) {
  const c=gcost(dx,dy,deg); if (c<gbest.c) gbest={dx,dy,deg,c}
}
console.log(`global fit: shift ${gbest.dx},${gbest.dy}px  rotate ${gbest.deg}deg  cost ${gbase.toFixed(2)} -> ${gbest.c.toFixed(2)} (${Math.round((1-gbest.c/gbase)*100)}% closer)`)
const gr=(gbest.deg*Math.PI)/180, gc=Math.cos(gr), gs=Math.sin(gr)
const proposals = blocks.map((block) => ({
  name: block.name, orig: block.rings, confident: true,
  prop: block.rings.map((ring) => ring.map(([lng,lat]) => {
    const x=px(lng), y=py(lat)
    const rx=gcx+(x-gcx)*gc-(y-gcy)*gs+gbest.dx, ry=gcy+(x-gcx)*gs+(y-gcy)*gc+gbest.dy
...
await sharp(satBuf).png().composite([{ input: svg('both') }]).toFile('.ui-check/g-oldvsnew.png')
await sharp(edgePng).composite([{ input: svg('both') }]).png().toFile('.ui-check/g-boundarymap.png')
await sharp(satBuf).png().composite([{ input: svg('prop') }]).toFile('.ui-check/g-proposed.png')
